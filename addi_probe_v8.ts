...
async function testTargeted(name, payload, extraHeaders = {}) {
    console.log(`\n--- Testing: ${name} ---`);
...
async function run() {
    ...
    await testTargeted("Minimal with header", minPayload);
    await testTargeted("Minimal without items.category", { ...minPayload, items: [{ sku: "1", name: "ITEM", quantity: 1, unitPrice: 100000 }] });
}
