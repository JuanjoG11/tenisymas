...
async function run() {
    await testVariation("totalAmount as string", { totalAmount: "150000" });
    await testVariation("items[0].unitPrice as string", { items: [{ sku: "SKU001", name: "ITEM", quantity: 1, unitPrice: "150000", category: "Fashion" }] });
    await testVariation("With administrativeDivision 'CUNDINAMARCA'", { shippingAddress: { line1: "CALLE 123", city: "BOGOTA", country: "CO", administrativeDivision: "CUNDINAMARCA" } });
    await testVariation("idNumber as integer (if possible)", { client: { ...{ idType: "CC", idNumber: 12345678, firstName: "JUAN", lastName: "PRUEBA", email: "juan.prueba@test.com", cellphone: "3001234567" } } });
}
